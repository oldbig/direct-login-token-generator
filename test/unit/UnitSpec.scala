...
class UnitSpec extends PlaySpec {
  ...
  "WidgetForm" must {
    ...
    "apply successfully from request" in {
      ...
      implicit val request: Request[_] = FakeRequest(call).withFormUrlEncodedBody("username" -> "foo", "password" -> "bar", "consumer_key" -> "my-consumer-key")
      ...
      widgetData.username must equal("foo")
      widgetData.password must equal("bar")
    }

    "apply successfully from map" in {
      // You can also bind directy from a map, if you don't have a request handy.
      val data = Map("username" -> "foo", "password" -> "bar", "consumer_key" -> "my-consumer-key")
      ...
      widgetData.username must equal("foo")
      widgetData.consumer_key must equal("my-consumer-key")
    }
